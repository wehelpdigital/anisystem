...
@foreach ($posts as $post)
    ...
    <article class="card p-3 sm:p-4 mb-4 sm:mb-5 group-post fp-card fp-hue-{{ $post->id % 6 }}"
        ...
        {{-- What the box is for, beside the face — an unlabelled field at the
             ...
             Four words is the whole label: the advice underneath said what to
             write and how to tag somebody, which is a paragraph to read before
             a one-line box, and the tagging half belongs where the typing
             happens rather than above it. --}}
        <p class="reply-lead">
            <span class="avatar avatar-sm {{ CommunityAvatar::hue(auth()->user()->full_name ?? '?') }}">{{ auth()->user()->initials ?? '?' }}</span>
            <span><b>Sagutin mo ito</b></span>
        </p>
