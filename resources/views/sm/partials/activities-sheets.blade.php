    ...
    <div class="sheet-body space-y-1">
        @php
            $actRows = [
                ...
                // What a week costs, asked of the day pills themselves.
                ['cashRangeBtn', 'Total two days', 'M12 1v22M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6', '', 'actCashRangeLabel'],
...
<div class="sheet hidden" id="dayMenuSheet" style="--sheet-width:24rem">
    ...
    <div class="sheet-body space-y-1">
        ...
        {{-- The camera is the fastest note anyone writes. Standing in front of
             ...
             exist. --}}
        {{-- Each answers to the tool that makes it, the way the drawing and
             the map below answer to theirs. They were hidden from every
             worker by a blanket flag, so an owner could hand somebody the
             camera and they would never find it on the day they were
             standing in. --}}
        @php
        ...
        {{-- These two answer to the module each one belongs to, and to it
             alone. Drawing on a day is the Drawing module's act and pinning a
             map to one is the Maps module's; the day note they land in is the
             file cabinet, not a second permission to ask for. A worker handed
             the Drawing pen and no notebook was being shown a button their
             own permission had already opened, greyed out by somebody else's.
             The save follows the same rule — see the inline-note.save line in
             WorkerModuleAccess. --}}
        @php
            $mayDrawHere = \App\Support\WorkerContext::canWriteModule('draw');
            $mayMapHere = \App\Support\WorkerContext::canWriteModule('maps');
        @endphp
        @if ($mayDrawHere)
        ...
        {{-- "Share this day" lived here too; sharing kept its own button on
             the day's header row, so the menu row was a second door. --}}
        @if (! \App\Support\WorkerContext::activeGrant() || $mayEdit)
