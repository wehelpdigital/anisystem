...
@php
    $noteMedia = collect(is_array($note->media) ? $note->media : [])
        ...
        ->filter()->values();
    // No worker moves a note, whatever their tier — the same question the
    // board's MAY_DRAG_NOTE asks, asked here so the grip is never painted in
    // the first place rather than swept off a moment later.
    $noteMayDrag = ! \App\Support\WorkerContext::activeGrant();
    // Same three answers, and the same words, the board's own sweep gives —
