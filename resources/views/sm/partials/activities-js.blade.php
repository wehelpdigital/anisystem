...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const SVG = {
        ...
        trash: '<svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 6h18M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2M10 11v6M14 11v6"/></svg>',
    ...
    function sayStockPick() {
        ...
        if (!sel.value) {
            ...
            /* Choosing "No" again must UNDO the shed's takeover: the name box
               ...
               branch used to return before the reveal code below ever ran,
               leaving a plain line with no name box at all. */
            $id('itemNameWrap')?.classList.remove('hidden');
            const nb = $id('itemNameInput');
            if (nb) nb.value = '';
            const tE = $id('itemStockIcon');
        ...
        /* The tag wears the pick. */
        const tagE = $id('itemStockIcon');
    ...
    /* The stock tag opens its sheet; a row sets the hidden select and lets
       the change handler do what it always did. */
    let LINE_KIN = null;
    $id('itemStockBtn')?.addEventListener('click', () => {
        const items = window.IV_ITEMS || [];
        const nowId = $id('itemStockPick')?.value || '';
        $id('itemStockList').innerHTML = `
            ...
            <button type="button" class="dt-row${nowId === String(i.id) ? ' is-on' : ''}" data-stock-row="${i.id}">
                ...
                <span class="dt-row-body"><b>${esc(i.name)}</b>
                    <i>${esc(i.says)} on hand · counted in ${esc(i.unitLabel || i.unit)}</i></span>
            </button>`).join('');
        openSheet('itemStockSheet');
    });
    $id('itemStockList')?.addEventListener('click', (e) => {
        const row = e.target.closest('[data-stock-row]');
        if (!row) return;
        const sel = $id('itemStockPick');
        sel.value = row.getAttribute('data-stock-row') || '';
        closeSheet('itemStockSheet');
        sayStockPick();
        syncToShedVisibility();
    });
    ...
    $id('itemUnitList2')?.addEventListener('click', (e) => {
        const row = e.target.closest('[data-unit-row]');
        if (!row) return;
        const says = row.getAttribute('data-unit-row');
    ...
    function lineUnitValue() {
        // The kin select is permanently hidden behind its tag; whether it is
        // on duty is whether a shed item is picked.
    ...
    function toggleCardExpand(card) {
        ...
        /* The box slides between two measured heights, and the content
         * fades and rises into it while it does.
         *
         * The slide alone was measured, on a phone, finding nine or ten of
         * the twelve heights that device can draw in the time — near enough
         * every frame. Removing it (which was tried) left exactly one: a
         * jump. So it stays, and the fade rides along to cover the frames it
         * cannot afford. Height is measured to measured because the card's
         * children cannot be re-wrapped: the markup is rendered twice, in
         * Blade and in JS, and both copies would have to change in step
         * forever. */
        quietTheBoard(340);
        clearTimeout(card.__foldTimer);
        card.classList.remove('is-revealing', 'is-hiding');
        ...
        card.classList.add(opening ? 'is-revealing' : 'is-hiding');
     ...
     * these rows.
     */
    async function moveExpenseToDate(src, toDate, orderIds) {
        ...
        try {
            ...
            /* The day it landed on answers in its own stored order, which
             * knows nothing about where the row was let go. `orderIds` is the
             * strip as the eye last saw it, so the row stays between the two
             * rows it was dropped between instead of jumping to the end. */
            if (orderIds && orderIds.length > 1) {
                const rows = DAY_EXPENSES[toDate];
                const seen = new Set();
                const ordered = [];
                orderIds.forEach((id) => {
                ...
                rows.forEach((r) => { if (!seen.has(r.id)) ordered.push(r); });
                DAY_EXPENSES[toDate] = ordered;
            }
    ...
    function restoreDraggedRow(row) {
        ...
        if (dxOrigin.next && dxOrigin.next.parentNode === dxOrigin.parent) {
            ...
            dxOrigin.parent.appendChild(row);
        }
        row.setAttribute('data-date', dxOrigin.date);
    }
    ...
    document.addEventListener('dragover', (e) => {
        ...
        if (dragDx) {
            ...
            $qsa('.date-group.drag-over-group').forEach((el) => el.classList.remove('drag-over-group'));
            if (overGroup && overDate && overDate !== '__no-date__' && moving) {
                e.preventDefault();
                if (e.dataTransfer) e.dataTransfer.dropEffect = 'move';
    ...
    document.addEventListener('drop', (e) => {
        ...
        /* An expense being put down.
         *
         * The row has been travelling with the cursor, so where it is IS the
